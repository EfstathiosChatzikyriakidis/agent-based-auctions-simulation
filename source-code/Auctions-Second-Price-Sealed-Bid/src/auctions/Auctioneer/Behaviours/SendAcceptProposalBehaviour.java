...
public class SendAcceptProposalBehaviour extends Behaviour {
    ...
    public void action() {
        ...
        if (bids.size() > 0) {
            ...
            int bidToPay = bestBid.getValue();

            Pair<AID, Integer> secondBestBid = bids.stream().filter(o -> o.getValue() < bestBid.getValue()).findFirst().orElse(null);

            if (secondBestBid != null) {
                bidToPay = secondBestBid.getValue();
            }
            ...
            orderMessage.setContent(item.getKey() + "," + bidToPay);

            orderMessage.setConversationId("bid-auction-second-price-sealed-bid");
