...
public class ReceiveCfpBehaviour extends CyclicBehaviour {
    ...
    public void action() {
        ...
        int bidderPrice = bidderEvaluation - 1;

        if (agent.getBudget() < bidderPrice) {
            replyMessage.setPerformative(ACLMessage.REFUSE);
        ...
        replyMessage.setContent(String.valueOf(bidderPrice));

        agent.send(replyMessage);

        logger.log(Level.INFO, "{0} : Sent proposal with ''{1}'' price.", new Object[] { agent.getLocalName(), bidderPrice });
    }
}